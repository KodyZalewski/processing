...
/**
 * @author kody
 * Manipulates data in the mri scan and performs basic adjustments. Smoothing, erosion, clean-up, basic template maps.  
 */


public class EditScan {
	
	static boolean BOUNDARIES[][][];
	
	// TODO: alter public methods to handle opposite dimensions independently
	// TODO: potentially consolidate with CalcScan.java
	// TODO: do we need Math.abs() every single time?
	...
	/**
	 * @author KJZ 1.7.2020
	 * @param Takes data as 3D matrix of doubles.
	 * @param Takes boolean nonZero, True = only non-zero voxels are included
	 * @result Finds total average of a nifti for given voxel set
	 */
	
	public static double totalAvg(double[][][] data, boolean nonZero) {
	...
	/**
	 * @author KJZ 12.19.2019
	 * @param Takes data as double 3D matrix.
	 * @param len is length as int, denotes how many voxels parallel to the given dim to smooth (default is 1).
	 * @return double 3D matrix written to a new scan as a smoothed volume. 
	 * @result reduces intensity outliers in scan, thresholding of non-brain voxels is more robust. 
	 */
	...
	public static double[][][] movingAvg(double[][][] data, int len) {
		...
		return movingAvgHelp(movingAvgHelp(movingAvgHelp(data, 
				z, y, x, len, "x"), 
				z, x, y, len, "y"),
				x, y, z, len, "z");
	}
	
	private static double[][][] movingAvgHelp(double[][][] data, 
	 ...
	 * @param int a, b, c are respective dimensions depending on dim (as string) being traversed.
	 * @param depth as int is how many voxels deep the sample is on scan periphery to 
	 * obtain an average estimation. (~3 is a robust number)
	 * @result finds average of the most peripheral voxels in dimension
	 * passed to the function. Aids with normalizing each dimension. 
	 */
	
	public static float distalAvg(double[][][] data, String dim, 
	...
	// finds average voxel intensity in six dimensional facets
	// depth denotes number of voxels included for calculation at each point at coordinate (x,y)
	public static float[] dimAvg(double[][][] data, int depth) {
		
		float[] avgs = new float[6];
		
		avgs[0] = distalAvg(data, "x", depth, 
				data.length-1, data[0].length-1, 0, 1, data[0][0].length/2);
		avgs[1] = distalAvg(data, "x", depth, 
				data.length-1, data[0].length-1, data[0][0].length-2, -1, data[0][0].length/2);		
		avgs[2] = distalAvg(data, "y", 
				depth, data.length-1, data[0][0].length-1, 0, 1, data[0].length/2);		
		avgs[3] = distalAvg(data, "y", 
				depth, data.length-1, data[0][0].length-1, data[0].length-2, -1, data[0].length/2);		
		avgs[4] = distalAvg(data, "z", 
				depth, data[0][0].length-1, data[0].length-1, 0, 1, data.length/2);	
		avgs[5] = distalAvg(data, "z", 
				depth, data[0][0].length-1, data[0].length-1, data.length-2, -1, data.length/2);
		System.out.println("X+ outer average is: " + CalcScan.round(avgs[0],3) + 
				" X- outer average is: " + CalcScan.round(avgs[1],3));
		System.out.println("Y+ outer average is: " + CalcScan.round(avgs[2],3) +  
				" Y- outer average is: " + CalcScan.round(avgs[3],3)); 
		System.out.println("Z+ outer average is: " + CalcScan.round(avgs[4],3) + 
				" Z- outer average is: " + CalcScan.round(avgs[5],3));
		System.out.println("");
	...
	private static float[][] histCalc(float avg, float[] floatArray, ArrayList<Float> totalVals) {
		...
		float stDev =  CalcScan.stDev(floatArray, avg);
	...
	public static double[][][] erode(Nifti1Dataset inputScan, double[][][] data, boolean x, boolean y, boolean z, 
		...
		System.out.println("Eroding " + erode + " voxels from the surface with "
				+ "intensities greater than " + CalcScan.round(lowBnd,2) + "...");
		...
		if (x && firstHalf) {
				data = erodeHelp(data, "x", erode, lowBnd, ZDIM, YDIM, XDIM, 0);
				dimensions = " +x,";
		}
		if (x && secondHalf) {
				data = erodeHelp(data, "x", erode, lowBnd, ZDIM, YDIM, XDIM, -(XDIM));
				dimensions = " -x,";
		}
		if (y && firstHalf) {
				data = erodeHelp(data, "y", erode, lowBnd, ZDIM, XDIM, YDIM, 0);
				dimensions = " +y,";
		}
		if (y && secondHalf) {
				data = erodeHelp(data, "y", erode, lowBnd, ZDIM, XDIM, YDIM, -(YDIM));
				dimensions = " -y,";
		}
		if (z && firstHalf) {
				data = erodeHelp(data, "z", erode, lowBnd, XDIM, YDIM, ZDIM, 0);
				dimensions = " +z,";
		}
		if (z && secondHalf) {
				data = erodeHelp(data, "z", erode, lowBnd, XDIM, YDIM, ZDIM, -(ZDIM));
				dimensions = " -z";
		}
	...
	private static double[][][] erodeHelp(double[][][] data, String dim, int erode, float lowBnd, 
	...
	public static double[][][] cleanUp(double[][][] data, int erode, 
		...
		if (x) {
			data = cleanUpHelp(data, "x", erode, simpleClean, ZDIM, YDIM, XDIM, 0);
			data = cleanUpHelp(data, "x", erode, simpleClean, ZDIM, YDIM, XDIM, -(XDIM));
		}
		if (y) {
			data = cleanUpHelp(data, "y", erode, simpleClean, ZDIM, XDIM, YDIM, 0);
			data = cleanUpHelp(data, "y", erode, simpleClean, ZDIM, XDIM, YDIM, -(YDIM));
		}
		if (z) {
			data = cleanUpHelp(data, "z", erode, simpleClean, XDIM, YDIM, ZDIM, 0);
			data = cleanUpHelp(data, "z", erode, simpleClean, XDIM, YDIM, ZDIM, -(ZDIM));
		}
	...
	private static double[][][] cleanUpHelp(double[][][] data, String dim, int erode, 
	...
	public static double[][][] distalClean(double[][][] data) {
		
		int XDIM = data[0][0].length-1; int YDIM = data[0].length-1; int ZDIM = data.length-1;
		
		data = distalCleanHelp(data, "x", ZDIM, YDIM, 0);
		data = distalCleanHelp(data, "x", ZDIM, YDIM, XDIM);
		data = distalCleanHelp(data, "y", ZDIM, XDIM, 0);
		data = distalCleanHelp(data, "y", ZDIM, XDIM, YDIM);
		data = distalCleanHelp(data, "z", XDIM, YDIM, 0);
		data = distalCleanHelp(data, "z", XDIM, YDIM, ZDIM);
		
		return data;
		
	}
	
	public static double[][][] distalCleanHelp(double[][][] data, String dim, int a, int b, int c) {
	...
	private static double[][][] patchSlice(double[][][] data, double[][][] dataBound, 
		...
		for ( int i = 0; i < a; i++ ) {
			dataBound = patchSliceHelp(data, patchSliceHelp(data, dataBound, 
					dim, 1, i, 0, 0, b, c), 
					dim, -1, i, b-1, c-1, 0, 0);
			
			dataBound = patchSliceHelp2(data, dim, i, b, c);
		}
	...
	private static double[][][] patchSliceHelp(double[][][] data, double[][][] dataBound, 
	...
	private static double[][][] patchSliceHelp2(double[][][] dataBound, String dim, int a, int j, int k) {
	...
	private static boolean checkNeg(int i, int j, int k) {
		if (i < 0 || j < 0 || k < 0) {
